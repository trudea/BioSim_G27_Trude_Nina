...
simulation = BioSim(default_txt, default_pop, default_seed)
herbivores = []
carnivores = []
years = 0
years_list = []
...
herbivore_line, = ax.plot(x, 0, 'g-')
carnivore_line, = ax.plot(x, 0, 'r-')
for i in range(20):
    simulation.one_year()
    years += 1
    herbivore = herbivores.append(sum(simulation.animal_distribution[
                                       'Herbivore']))
    carnivore = carnivores.append(sum(simulation.animal_distribution[
                                       'Herbivore']))
    years_list.append(years)
    herbivore_line.set_ydata(herbivore)
    carnivore_line.set_ydata(carnivore)
    fig.canvas.draw()
    plt.pause(1)


def heatmap():
    x = sim.animal_distribution
    y = x.pivot('Row', 'Col', 'Herbivore')
    ax = sns.heatmap(y, vmax=200)
    plt.show()
